		...
		if (fifo32_status(&task->fifo) == 0) {
			...
			if (i <= 1) { /* ƒJ[ƒ\ƒ‹—pƒ^ƒCƒ} */
				if (i != 0) {
					...
					timer_init(cons.timer, &task->fifo, 1); /* ŽŸ‚Í1‚ð */
					if (cons.cur_c >= 0) {
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
	...
	name[i] = 0; /* ‚Æ‚è‚ ‚¦‚¸ƒtƒ@ƒCƒ‹–¼‚ÌŒã‚ë‚ð0‚É‚·‚é */

	/* ƒtƒ@ƒCƒ‹‚ð’T‚· */
	finfo = file_search(name, (struct FILEINFO *) (ADR_DISKIMG + 0x002600), 224);
	if (finfo == 0 && name[i - 1] != '.') {
		/* Œ©‚Â‚©‚ç‚È‚©‚Á‚½‚Ì‚ÅŒã‚ë‚É".HRB"‚ð‚Â‚¯‚Ä‚à‚¤ˆê“x’T‚µ‚Ä‚Ý‚é */
		name[i    ] = '.';
		name[i + 1] = 'H';
		name[i + 2] = 'R';
		name[i + 3] = 'B';
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		sheet_updown(sht, shtctl->top); /* ¡‚Ìƒ}ƒEƒX‚Æ“¯‚¶‚‚³‚É‚È‚é‚æ‚¤‚ÉŽw’èF ƒ}ƒEƒX‚Í‚±‚Ìã‚É‚È‚é */
		reg[7] = (int) sht;
	} else if (edx == 6) {
		...
		for (;;) {
			...
			io_sti();
			if (i <= 1) { /* ƒJ[ƒ\ƒ‹—pƒ^ƒCƒ} */
				/* ƒAƒvƒŠŽÀs’†‚ÍƒJ[ƒ\ƒ‹‚ªo‚È‚¢‚Ì‚ÅA‚¢‚Â‚àŽŸ‚Í•\Ž¦—p‚Ì1‚ð’•¶‚µ‚Ä‚¨‚­ */
				timer_init(cons->timer, &task->fifo, 1); /* ŽŸ‚Í1‚ð */
				timer_settime(cons->timer, 50);
			}
			if (i == 2) {	/* ƒJ[ƒ\ƒ‹ON */
		...
		((struct TIMER *) reg[7])->flags2 = 1;	/* Ž©“®ƒLƒƒƒ“ƒZƒ‹—LŒø */
	} else if (edx == 17) {
